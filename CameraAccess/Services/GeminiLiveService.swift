...
class GeminiLiveService: NSObject {
    ...
    func connect() {
        ...
        print("🔌 [Gemini] API key length: \(apiKey.count), prefix: \(String(apiKey.prefix(8)))...")
        ...
        print("🔌 [Gemini] WebSocket task started, URL: \(baseURL)")
        receiveMessage()
    }
    ...
    private func configureSession() {
        ...
        // Gemini Live API setup message (v1beta uses "setup" with snake_case)
...
extension GeminiLiveService: URLSessionWebSocketDelegate {
    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        if let error = error {
            print("❌ [Gemini] URLSession task failed: \(error.localizedDescription)")
